...
public class TSPLibLoader 
{
	...
	{//Load in a TSPLib instance. This example assumes that the Edge weight type 
		...
		try
		{
			String currentLine;
			int dimension=0;
			//Hold the dimension of the problem
			...
			while((currentLine= br.readLine()) != null) 
			{
				...
				if(readingNodes)
				{
					//If reading in the node 
					String[] tokens = currentLine.split(" ");
					//Split the line by spaces.
					//tokens[0] is the city id and not needed in this example
					float floatx = Float.parseFloat(tokens[1].trim());
					float floaty = Float.parseFloat(tokens[2].trim());
					//Use Java's built in Point2D type to hold a cityPoint2D 
					City city = new City((int)floatx, (int)floaty);
